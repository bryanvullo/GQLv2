...
tokens :-
 ...
 NEW                             { \p _ -> Tn TokenNew p }
 [a-z]+\:string                  { \p s -> Tn (TokenStringField (take ((length s) - 7) s)) p }
 [a-z]+\:boolean                 { \p s -> Tn (TokenBoolField (take ((length s) - 8) s)) p }
 [a-z]+\:integer                 { \p s -> Tn (TokenIntField (take ((length s) - 8) s)) p }
...
data TokenBase =
 ...
 TokenExclude |
 TokenNew |
 TokenStringField String |
 TokenBoolField String |
 TokenIntField String
  deriving (Eq, Show)
}
