...
groupNodesToTables :: [[(String, String, GraphValue)]] -> Map String (String, [(String, String)])
groupNodesToTables nodes = Map.fromListWith (++) [(getHeader node, [(getHeader node, nodeToRow node)]) | node <- nodes]
    where
        getHeader = intercalate "," . map (\(k, t, _) -> k ++ ":" ++ t) . takeWhile (\(k, _, _) -> k /= "LABEL")
        nodeToRow = intercalate "," . map (\(k, _, v) -> valueToString v)
        valueToString (I x) = show x
        valueToString (S x) = x
        valueToString (Ss xs) = intercalate ";" xs
        valueToString (B True) = "true"
